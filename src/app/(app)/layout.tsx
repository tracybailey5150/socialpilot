    ...
    <div style={{ minHeight: '100vh', background: bg, color: text, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px', display: 'flex', flexDirection: 'column', gap: '5px', zIndex: 101 }}
        >
      ...
      <div style={{
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.href === '/generate' && (
                  <span style={{ marginLeft: 'auto', background: 'rgba(16,185,129,0.2)', color: accent, fontSize: '11px', fontWeight: 700, padding: '2px 8px', borderRadius: '4px' }}>NEW</span>
                )}
              </Link>
            );
          })}
        ...
        <div style={{ borderTop: `1px solid ${border}`, padding: '20px' }}>
          <div style={{ fontSize: '12px', color: muted, marginBottom: '12px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {userEmail}
          </div>
          <button
            onClick={handleSignOut}
            style={{ width: '100%', background: 'transparent', border: `1px solid ${border}`, borderRadius: '8px', color: muted, padding: '12px', fontSize: '14px', cursor: 'pointer' }}
          >
            Sign Out
          </button>
        </div>
